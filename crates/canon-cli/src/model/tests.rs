...
//! one-connection-per-response local listener recording bodies — reimplemented
//! over `std::net` because `canon` has no async runtime and is not getting one.
...
impl Mock {
    /// Serve exactly `script.len()` requests, in order.
