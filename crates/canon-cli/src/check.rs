...
//! each pulls, and why. [`Standing::cited`] then refuses any bearing naming a
...
use canon_core::{Bearing, Canon, Commitment, Disposition, Outcome, Pull, Standing, Status};
...
pub fn assess(
    ...
    canon: &Canon,
    proposal: &str,
) -> Result<(Standing, Vec<Bearing>), ModelError> {
    ...
    for b in judged.bearings {
        ...
        let Some(c) = (b.commitment >= 1)
            ...
            eprintln!(
                "warning: dropped a bearing naming commitment {} — only 1..{} exist",
                b.commitment,
                live.len()
            );
            continue;
        ...
        let pull = match b.pull.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                eprintln!("warning: dropped a bearing with an unreadable pull `{other}`");
                continue;
            }
        };
        bearings.push(Bearing {
            ...
            pull,
            because: b.because.trim().to_string(),
        });
    }
    Ok(Standing::cited(canon, proposal, bearings))
}
...
fn cite(canon: &Canon, b: &Bearing, indent: &str) -> String {
    let Some(c) = canon.get(&b.commitment) else {
        return String::new();
    };
...
fn carried(canon: &Canon, standing: &Standing) -> Vec<String> {
    ...
    for c in canon.tolerated() {
        let both = standing.bearings.iter().any(|b| b.commitment == c.a)
            && standing.bearings.iter().any(|b| b.commitment == c.b);
        if !both {
            continue;
        }
...
fn render_code(canon: &Canon, standing: &Standing) -> String {
    ...
    match standing.outcome() {
        Outcome::Conflicts => {
            ...
            let toward: Vec<&Bearing> = standing.toward().collect();
            if !toward.is_empty() {
...
fn render_house(canon: &Canon, standing: &Standing) -> String {
    ...
    match standing.outcome() {
        Outcome::Conflicts => {
            ...
            for b in standing.against() {
                out.push_str(&cite(canon, b, "  "));
                if let Some(c) = canon.get(&b.commitment) {
                    out.push_str(&format!(
...
fn render_personal(canon: &Canon, standing: &Standing) -> String {
    ...
    let width = standing
        .bearings
        .iter()
        .filter_map(|b| canon.get(&b.commitment))
        .map(|c| c.text.chars().count())
        .max()
        .unwrap_or(0)
        .min(52);
    for b in &standing.bearings {
        let Some(c) = canon.get(&b.commitment) else {
            continue;
        };
...
pub fn run(args: &[String]) -> i32 {
    ...
    for b in &refused {
        eprintln!(
            "warning: refused an uncitable bearing on {} — {}",
            b.commitment,
