...
| 3 | Collective-choice arrangements | policy and **ratification** in the ledger | the cooks change the kitchen's own rule, and the next decision goes the other way on the same evidence; then they set how kitchen rules are *made* — both cooks, jointly — and a non-cook's rule is a **proposal** until both say yes; the garden goes further and carries a rule **twice with somebody new in between**, and the change to its rule is itself judged under the rule it replaces | mechanism |
